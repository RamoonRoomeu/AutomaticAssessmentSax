...
class SolfegeGradeEstimator(object):
    ...
    def get_syllable_seq_ref(self, input_audio_filename_ref):
        """
        :param input_audio_filename_ref: wav full filename, reference
        :return:
        """
        fn = os.path.basename(input_audio_filename_ref)
        fn = os.path.splitext(fn)[0]

        syllables_ref = syllable_sequence_alignment(kaldi_root=self.kaldi_root,
    ...
    def reference_feature_extraction(self, filename_input_ref_audio):
        ...
        audio_ref = self.audio_resample(input_audio_filename=filename_input_ref_audio)

        # ref syllable seq
        syllable_seq_ref, onset_ref = self.get_syllable_seq_ref(input_audio_filename_ref=filename_input_ref_audio)

        self.boundaries_ref = self.onsets_2_boundaries(self.onsets_ref)
        self.timestamps_ref, self.melody_cents_ref = get_pyin_pitchtrack(audio=audio_ref, fs=44100)

        self.median_pitch_ref_syllable = get_median_syllable_pitch(time_stamps=self.timestamps_ref,
                                                                   pitch_track=self.melody_cents_ref,
                                                                   boundaries=self.boundaries_ref)
