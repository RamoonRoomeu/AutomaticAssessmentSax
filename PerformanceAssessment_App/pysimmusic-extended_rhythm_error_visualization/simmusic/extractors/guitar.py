...
def save_bar_plot(
        ...
        segments=np.array([], dtype=float), devs=np.array([], dtype=float),
        misses=np.array([], dtype=float), excessives=np.array([], dtype=float),
    ...
    filtered_misses = misses[(misses >= left_time) & (misses < right_time)]
    filtered_excessives = excessives[(excessives >= left_time) & (excessives < right_time)]
    filtered_devs = []
    filterted_segments = []
    for i in range(len(segments)):
        s = segments[i]
        dev = devs[i]
        if s[0] >= left_time or s[1] <= right_time:
            filtered_devs.append(dev)
            filterted_segments.append((max(left_time, s[0]), min(right_time, s[1])))

    # reference notes
    for x in filtered_expected:
        plt.axvline((x - left_time) * fs, color='k', lw=0.5)
    # segments
    for i in range(len(filterted_segments)):
        s = filterted_segments[i]
        rgbas = attack_color_func(abs(filtered_devs[i]))
        plt.axvspan((s[0] - left_time) * fs, (s[1] - left_time) * fs, facecolor=rgbas[0], alpha=0.5)
    # misses
    err_color = attack_color_func(100)[0]
    for m in filtered_misses:
        plt.text((m - left_time) * fs, 0, '0',
                 horizontalalignment='center',
                 verticalalignment='center',
                 fontsize=10, color=err_color)
    # excessives
    for e in filtered_excessives:
        plt.axvline((e - left_time) * fs, color=err_color, lw=0.5)
        plt.text((e - left_time) * fs, 0, 'X',
                 horizontalalignment='center',
                 verticalalignment='center',
                 fontsize=10, color=err_color)
...
def assess_guitar_exercise(
    ...
    img_file_name = visualize(
        ...
        segments=timing_features['segments'],
        devs=timing_features['devs'],
        misses=timing_features['misses'],
        excessives=timing_features['excessives'],
